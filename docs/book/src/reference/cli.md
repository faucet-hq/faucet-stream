...
| `--source-hub` / `--sink-hub` / `--overlay-hub <hub>` | *(run / validate / compose / check)* Look up that side in its own hub instead of the `--hub` list. A single locator can also name its hub inline: `--source github:acme/private-hub:acme/netsuite` (hub, colon, id). The composed config's header comment records which hub each side came from. |
