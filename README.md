...
| **Embedded integrations (multi-tenant)** | Run the same pipeline for every customer: tenants with limits, credentials sealed under a vault key (stored by API or created through a hosted OAuth connect flow with PKCE), `auth: { ref }` resolved per tenant, `${tenant.*}` routing, per-tenant state namespaces, fan-out on demand or on a cron `schedule` trigger, re-authorization detection that pauses only the affected tenant, tenant-confined tokens and a delete that removes everything faucet holds. See the [embedded integrations cookbook](https://faucet-hq.github.io/faucet-stream/cookbook/embedded-integrations.html). |
